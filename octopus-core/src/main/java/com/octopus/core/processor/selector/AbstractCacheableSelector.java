 ...
 * @date 2024/01/12
 */
public abstract class AbstractCacheableSelector<T, P extends AbstractSelectorProperties> extends AbstractSelector<P> {
    private final LRUCache<String, T> cache;

    public AbstractCacheableSelector() {
        this.cache = new LRUCache<>(16);
    }

    @Override
    protected List<String> doMultiSelect(String source, P selector, Response response)
    ...
    protected abstract List<String> doSelectWithDoc(
            T t, P selector, boolean multi, Response response) throws SelectException;
