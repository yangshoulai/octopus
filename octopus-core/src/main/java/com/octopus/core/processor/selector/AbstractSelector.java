...
public abstract class AbstractSelector<P extends AbstractSelectorProperties> implements Selector<P> {
    ...
    public List<String> select(String source, boolean multi, P selector, Response response)
        ...
        try {
            ...
            if (selector.getDenoiser() != null) {
                selected = this.applyDenoiser(selector.getDenoiser(), selected);
            }
    ...
    private List<String> applyDenoiser(DenoiserProperties denoiser, List<String> selected) {
        if (denoiser.isSplit()) {
            selected =
                    selected.stream()
        ...
        if (StrUtil.isNotBlank(denoiser.getRegex())) {
            selected =
                    selected.stream()
                            ...
                            .collect(Collectors.toList());
        } else if (StrUtil.isNotBlank(denoiser.getFormat())) {
            selected = selected.stream().map(s -> String.format(denoiser.getFormat(), s)).collect(Collectors.toList());
        }
        return selected;
    }
